import pytest
from sympy import Poly, symbols, sympify, zoo

from src.sintesis_redes import (
    CauerI,
    CauerII,
    FosterI,
    FosterII,
...
def test_construir_polinomio():
    redes = [
        (
            SintesisRedes.construir_polinomio_complejos_conjugados([1, 2, 0], [3, 4]),
            (s**2 + 1) * (s**2 + 2) * s / (s**2 + 3) / (s**2 + 4),
        ),
        (
            SintesisRedes.construir_polinomio_complejos_conjugados([1, 4], [2, 8, 0]),
            (s**2 + 1) * (s**2 + 4) / (s**2 + 2) / (s**2 + 8) / s,
        ),
    ]
...
def test_fosterI_residuos():
    redes = [
        ...
        (
            FosterI([(3 + 5 ** (1 / 2)) / 2, (3 - 5 ** (1 / 2)) / 2], [0, 2], 1),
            [1 / 2, 1 / 2, 1],
        ),
...
def test_fosterI_RC_elementos():
    redes = [
        ...
        (
            FosterIRC([1, 3], [0, 2, 4], 10),
            [4 / 15, 5 / 2 / 2, 2 / 5, 15 / 4 / 4, 4 / 15, 0],
        ),
    ]
    for red, resultado_esperado in redes:
        assert red.elementos() == pytest.approx(resultado_esperado)
...
def test_fosterII_RC_elementos():
    redes = [(FosterIIRC([1, 3], [2]), [2 / 3, 1 / 4, 2, 1])]
    for red, resultado_esperado in redes:
        assert red.elementos() == pytest.approx(resultado_esperado)
...
def test_fosterI_RL_elementos():
    redes = [
        ...
        (
            FosterIRL([7.959, 0.484, 1.557], [1, 3], 5),
            [9.99634, 5.00025, 5.00025, 15.0034 / 3, 15.0034, 5],
        ),
...
def test_cauer_I():
    redes = [
        (
            CauerI(s * (s**2 + 4) * (s**2 + 25), (s**2 + 1) * (s**2 + 9)),
            [1, 1 / 19, 19**2 / 99, 11**2 * 9 / (640 * 19), 640 / 99],
        ),
        (
            CauerI((s**2 + 1) * (s**2 + 4), s * (s**2 + 2) * (s**2 + 8)),
            [0, 1, 0.2, 25 / 13, 169 / 280, 14 / 13],
        ),
        (
            CauerI((s**5 + 8 * s**3 + 12 * s), (s**4 + 5 * s**2 + 4)),
            [1, 1 / 3, 9 / 7, 49 / 60, 5 / 7],
        ),
    ...
    for red, resultado_esperado in redes:
        ...
        assert [
            Poly(elemento, s).all_coeffs()[0] for elemento in red.residuos()
        ] == pytest.approx(resultado_esperado)
...
def test_cauer_II():
    redes = [
        ...
        (
            CauerII(10 * (s + 1) * (s + 3), s * (s + 2) * (s + 4)),
            [30 / 8, 32 / 70, 490 / 88, 484 / 105, 15 / 22],
        ),
        (
            CauerII((s * (s**2 + 2) * (s**2 + 6)), (s**2 + 1) * (s**2 + 4)),
            [0, 1 / 3, 36 / 7, 49 / 96, 3072 / 105, 15 / 96],
        ),
        (
            CauerII((10 * s**5 + 60 * s**3 + 80 * s), (s**4 + 4 * s**2 + 3)),
            [0, 3 / 80, 320 / 7, 49 / 880, 19360 / 42, 6 / 880],
        ),
...
def extract_coefficients(elementos):
    coeffs_values = [
        list(sympify(elemento).as_coefficients_dict().values())
