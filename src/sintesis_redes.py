...
class FosterI(RedSintetizada):
    def __init__(self, ceros, polos, A=1):
        ...
        self._ceros = ceros
        self._polos = polos
        self._polinomio = SintesisRedes.construir_polinomio_complejos_conjugados(
            ceros, polos, A
        )
...
class CauerII(RedSintetizada):
    ...
    def sintetizar(self):
        ...
        while residuo != 0:
            ...
            min_degree_numerador = min(
                [k.as_coeff_exponent(self._s)[1] for k in numerador_coeffs.keys()]
            )
            min_degree_denominador = min(
                [k.as_coeff_exponent(self._s)[1] for k in denominador_coeffs.keys()]
            )
            ...
            min_coeff_denominador = denominador_coeffs.get(
                self._s**min_degree_denominador, 0
            )
            ...
            if min_degree_numerador > min_degree_denominador:
                ...
                cociente = (
                    min_coeff_numerador
                    / min_coeff_denominador
                    * self._s ** (min_degree_numerador - min_degree_denominador)
                )
